...
# Created on: Dec 2021
# This program translates a string to hexidecimal
...
def hex_translator(user_string):
    ...
    for word in word_list:
        for char in split(word):
            if char in dictionary.keys():
                transleated_string.append(dictionary[char])
            else:
                print("Invalid input")
    return transleated_string


def main():
    transleated_string = []
    user_string = input("Type something: ")
    hex_string = hex_translator(user_string)
    print("\n{0} in Hex is {1}.".format(user_string, hex_string))
    print("\nDone.")
